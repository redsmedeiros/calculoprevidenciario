...
export class ImportadorCnisPdfLoadComponent implements OnInit {
   ...
   */
  private selecionarDadosDoCliente(text) {
   ...
   * @param segurado obj segurado
   */
  private selecionarDadosDoVinculo(text_vinculo, segurado) {
  ...
  private selecionarDadosDosVinculos(seq, segurado) {
    ...
    seq.forEach(text_vinculo => {
      const vinculo_seq = this.selecionarDadosDoVinculo(text_vinculo, segurado);
    ...
    function filterPeriodos(vinculo) {
      return (vinculo.contribuicoes.length > 0 || vinculo.periodo != null || vinculo.tipoVinculo != null);
    }
  ...
  private newImportText(file_text) {
    ...
    if (text) {
      ...
      this.segurado = this.selecionarDadosDoCliente(text);
